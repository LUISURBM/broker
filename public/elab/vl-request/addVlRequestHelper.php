...
try {
    ...
    if(!empty($existResult)){
        if(isset($_POST['sampleCodeKey']) && !empty($_POST['sampleCodeKey']))
        {
            $sCode = intval($_POST['sampleCodeKey']) + 1;
...
//           'sample_code'=>(isset($_POST['sampleCode']) && $_POST['sampleCode']!='') ? $_POST['sampleCode'] :  NULL,
          ...
          'picked_by'=>(isset($_POST['specimenPickedBy']) && $_POST['specimenPickedBy']!='') ? $_POST['specimenPickedBy'] :  NULL,
          'patient_location'=>(isset($_POST['uniqueIdentifier']) && $_POST['uniqueIdentifier']!='') ? $_POST['uniqueIdentifier'] :  NULL,
        ...
        $existSampleQuery2 = "SELECT patient_art_no FROM vl_request_form WHERE patient_art_no='".trim($_POST['artNo'])."'";
//         echo $existSampleQuery2; die;
        $existResult = $db->rawQuery($existSampleQuery2);
        if($_POST['isPatientNew'] == 'no')
        {
            if(empty($existResult))
            {
                $_SESSION['alertMsg']="The patient you are editing does not exist.";
                header("location:addVlRequest.php"); exit;
            }
            else 
            {
                //$db->where('patient_art_no', trim($_POST['artNo']));
                //$update_result = $db->update($tableName, $vldata);
        ...
        else  // adding new patient
        {
            if(!empty($existResult))
            {
                $_SESSION['alertMsg']="ART No. duplicated for adding New Patient";
                header("location:addVlRequest.php"); exit;
            }
            else
            {
            ...
            if($id > 0)
            {
                ...
                if(isset($_POST['printBarCode']) && $_POST['printBarCode'] =='on'){
                 ...
                 if(isset($_POST['saveNext']) && $_POST['saveNext']=='next'){
                     ...
                     header("location:vlRequest.php"); exit;
                 }
            }
            else 
            {
                $_SESSION['alertMsg']="Add request failed.";
                echo "Add request failed.";
                error_log('Add request failed. Error = ' . $db->getLastError());
                error_log('query = ' . $db->getLastQuery());
                error_log('Error No. = ' . $db->getLastErrno());
//                 header("location:addVlRequest.php"); exit;
            }
        }
        else // $_POST['isPatientNew'] == 'no'
        {
            if($update_result)
            {
                ...
                $action = ucwords($_SESSION['userName']).' updated with a request data with the sample code '.$_POST['sampleCode'];
                $resource = 'vl-request-rwd';
                $data=array(
                    'event_type'=>$eventType,
                    'action'=>$action,
            ...
            {    
                $_SESSION['alertMsg']="Update request failed.";
                echo "Update request failed." .  $db->getLastQuery(); die;
                error_log('Update request failed. Error = ' . $db->getLastError());
                error_log('query = ' . $db->getLastQuery());
                error_log('Error No. = ' . $db->getLastErrno());
