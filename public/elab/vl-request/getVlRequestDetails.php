...
$primaryKey="vl_sample_id";
        ...
        */
        $aColumns = array('vl.serial_no',"DATE_FORMAT(vl.sample_collection_date,'%d-%b-%Y')",'b.batch_code','vl.patient_art_no','vl.patient_first_name','vl.patient_middle_name','vl.patient_last_name','f.facility_name','f.facility_state','f.facility_district','s.sample_name','vl.result','ts.status_name');
        $orderColumns = array('vl.serial_no','vl.sample_collection_date','b.batch_code','vl.patient_art_no','vl.patient_first_name','vl.patient_middle_name','vl.patient_last_name','f.facility_name','f.facility_state','f.facility_district','s.sample_name','vl.result','ts.status_name');
	...
	$_SESSION['vlRequestSearchResultQuery'] = $sQuery;
        ...
        $aResultFilterTotal =$db->rawQuery("SELECT vl.vl_sample_id,vl.facility_id,vl.patient_first_name,vl.patient_middle_name,vl.patient_last_name,vl.result,f.facility_name,f.facility_code,vl.patient_art_no,s.sample_name,b.batch_code,vl.sample_batch_id,ts.status_name FROM vl_request_form as vl LEFT JOIN facility_details as f ON vl.facility_id=f.facility_id LEFT JOIN r_sample_type as s ON s.sample_id=vl.sample_type INNER JOIN r_sample_status as ts ON ts.status_id=vl.result_status LEFT JOIN batch_details as b ON b.batch_id=vl.sample_batch_id $sWhere ORDER BY vl.last_modified_datetime DESC, $sOrder");
	...
	$vlRequest = false;
	$vlView = false;
	if((isset($_SESSION['privileges']) && (in_array("editVlRequest.php", $_SESSION['privileges']))) || $_SESSION['roleId'] == 2){
	    $vlRequest = true;
	}
	if((isset($_SESSION['privileges']) && (in_array("viewVlRequest.php", $_SESSION['privileges']))) || $_SESSION['roleId'] == 2){
	    $vlView = true;
	}
	    ...
	    $row[] = $aRow['patient_art_no'];
         ...
         $row[] = ucwords($aRow['patient_last_name']);
         $row[] = ucwords($aRow['patient_middle_name']);
		...
		if($aRow['vlsm_country_id']==2){
			if($vlRequest){
				$edit='<a href="editVlRequest.php?id=' . base64_encode($aRow['vl_sample_id']) . '&sid='.$aRow['serial_no'].'" class="btn btn-primary btn-xs" style="margin-right: 2px;" title="Edit"><i class="fa fa-pencil"> Edit</i></a>';
			...
			if($vlView){
			    $view = '<a href="viewVlRequestZm.php?id=' . base64_encode($aRow['vl_sample_id']) . '&sid='.$aRow['serial_no'].'" class="btn btn-default btn-xs" style="margin-right: 2px;" title="View"><i class="fa fa-eye"> View</i></a>';
			}
		}else{
			if($vlRequest){
			    $edit='<a href="editVlRequest.php?id=' . base64_encode($aRow['vl_sample_id']) . '&sid='.$aRow['serial_no'].'" class="btn btn-primary btn-xs" style="margin-right: 2px;" title="Edit"><i class="fa fa-pencil"> Edit</i></a>';
			...
			if($vlView){
			    $view = '<a href="viewVlRequest.php?id=' . base64_encode($aRow['vl_sample_id']) . '&sid='.$aRow['serial_no'].'" class="btn btn-default btn-xs" style="margin-right: 2px;" title="View"><i class="fa fa-eye"> View</i></a>';
			}
